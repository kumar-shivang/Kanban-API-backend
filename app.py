...
class User(db.Model):
    ...
    password_hash = db.Column(db.String(128)) # password hash is 128 characters long because it is generated by the werkzeug security module that returns a 128 character hash
    email = db.Column(db.String(50), unique=True) # email is unique. It has 50 characters because it is a optimum number of characters for an email address.
    userLists = db.relationship('List', backref='user', lazy='dynamic') # userLists is a relationship field that references the List class. It is used to get all the lists for a user.
...
# The List class represents a list in the database. It has 3 fields: listID, name and userID. The userID field is a foreign key that references the userID field in the User class.
class List(db.Model):
    ...
    listname = db.Column(db.String(30)) # name is a string with a maximum of 30 characters.
    userID = db.Column(db.Integer, db.ForeignKey('user.userID')) # userID is a foreign key that references the userID field in the User class
